...
export async function POST(request) {
  try {
    ...
    // ── 2. Nodemailer transporter (IPv4 forced to fix ECONNREFUSED) ────────
    ...
    // ── 3. Emails wrapped in try/catch so a mail failure never blocks the lead ──
    try {
      // Admin notification
      ...
      console.error("Email sending failed (lead still saved to DB):", mailErr.message);
    }
